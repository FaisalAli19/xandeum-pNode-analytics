...
export const getUptimeBadge = (uptime: number): 'success' | 'warning' | 'error' => {
...
export const getBadgeTextColor = (
  ...
  if (badgeType === 'success')
    return isDarkMode ? 'var(--chakra-colors-green-500)' : 'var(--chakra-colors-green-900)';
  // Warning badges: red in light mode, white in dark mode
  if (badgeType === 'warning') return isDarkMode ? 'white' : 'var(--chakra-colors-red-500)';
  // Error badges: red in light mode, white in dark mode
  if (badgeType === 'error') return isDarkMode ? 'white' : 'var(--chakra-colors-red-500)';
  return 'var(--chakra-colors-gray-400)';
};
