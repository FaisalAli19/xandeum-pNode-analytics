...
const API_URL = import.meta.env.VITE_APP_API_URL || 'https://api.devnet.xandeum.com:8899';
const PROGRAM_ID =
  import.meta.env.VITE_APP_PROGRAM_ID || '6Bzz3KPvzQruqBg2vtsvkuitd6Qb4iCcr5DViifCwLsL';
 ...
 */
export const fetchProgramAccounts = async (useJsonParsed = true): Promise<ProgramAccount[]> => {
  console.log(
    `🔄 Fetching pNodes from RPC (encoding: ${useJsonParsed ? 'jsonParsed' : 'base64'})...`
  );
  ...
  try {
    ...
    if (useJsonParsed && response.data.result.length > 0) {
      ...
      if (!hasParsedData) {
        console.warn('⚠️ jsonParsed encoding returned unparsed data, falling back to base64...');
        return fetchProgramAccounts(false);
      }
